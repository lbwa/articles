...
创造型设计模式（`Creational design patterns`）侧重管理对象的创建机制（`object creation mechanisms`），以适应我们当前工作情况的方式创建对象。基础的创建对象的方法在一定程度上会给当前项目增加复杂度（`complexity`）。同时创造型设计模式的目标时通过掌控创建的进程（`controlling the creation process`）从而解决复杂度提升的问题。

一些常见的创造型设计模式由 `Constructor`， `Factory`， `Abstract`， `Prototypes`， `Singleton`， `Builder` 等。

| 创造型设计模式 | 以创建一个具体（相对于抽象来说）对象为基础 |
| --- | --- |
...
结构型设计模式（`Structural design pattern`）关注对象的合成（`object composition`），通常用于实现不同对象之间的关系识别。该设计模式帮助我们确保当一个系统的某一部分改变时，不会给整个系统内的其他部分带来副作用。该设计模式也可以通过重构将不适应于系统的特定目的的部分转变为系统内具有
特定目的的一部分。

该类型设计模式包含：`Decorator`， `Facade`， `Flyweight`， `Adapter`， `Proxy`。

| 结构型设计模式 | 以创建封闭对象的思想为基础 |
| ------------ | ------------------------ |
| Adapter | 匹配不同类的接口，以使得这些可能具有互不兼容的接口的类能够一起工作。 |
| Bridge | 从已知的实现中分离一个对象的接口，以使得这两部分变得各自独立（解耦）。 |
| Composite | 一种可以使得合成对象的工作效果大于将各个部分简单相加的结构。 |
| Decorator | 向对象动态添加可选的进程。 |
| Facade | 一个隐藏了整个副系统的复杂度的简单类。即抽象副系统的接口。 |
| Flyweight | 一种细粒度实例，用于共享系统其他部分所包含的信息。 |
| Proxy | 一个用于表示真正对象的占位符对象。 |

- 行为型设计模式

行为型设计模式（`Behavioral design pattern`）聚焦改善或精简系统内的分离对象之间的通信。

常见的行为型设计模式有：`Iterator`，`Mediator`，`Observer`，`Visitor`。

| 结构型设计模式 | 以创建封闭对象的思想为基础 |
| ------------ | ------------------------ |
| Interpreter | 一种在应用中包含语言元素的方法，用于匹配目标语言的语法。 |
| Template Method | 在一个方法内创建一个算法 `shell`，然后将确切的步骤推迟到子类实现。|
| Chain of Responsibility | 在对象链中传入一个请求来定位监听该请求的一种方法。|
| Command | 将一个命令请求封装为一个启用的对象，以记录并（或）将该请求加入队列，并为未监听该请求的元素提供错误处理。 |
| Iterator | 在不知道内部工作原理的情况下，序列化访问集合中的元素 |
| Mediator | 定义了多个类之间的通信渠道，以防止一组类显式地相互引用（即防止耦合）。 |
| Memento | 捕获一个对象的内部状态，以用于之后能够恢复该状态。 |
| Observer | 一个用于通知类中一个成员的改变的方法，用于确保类之间的一致性。 |
| State | 在一个对象的状态 `state` 改变时，改变对象的行为。 |
| Strategy | 将一个算法封装在一个类中，该类将选择和实现分离解耦。 |
| Visitor | 在没有改变类的情况下向该类添加一个新的操作。 |
